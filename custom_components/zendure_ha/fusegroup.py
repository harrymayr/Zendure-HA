    ...
    def charge_limit(self, d: ZendureDevice) -> int:
        ...
        if self.initPower:
            ...
            if len(self.devices) == 1:
                d.pwr_max = min(self.minpower, d.charge_limit)
            else:
                used = 0
                weight = 0
                ...
                used = max(0, self.minpower - used)
                for fd in self.devices:
                    if fd.homeInput.asInt > 0:
                        fd.pwr_max = fd.charge_start + int(used * (fd.electricLevel.asInt * fd.actualKwh) / weight)
    ...
    def discharge_limit(self, d: ZendureDevice) -> int:
        ...
        if self.initPower:
            ...
            if len(self.devices) == 1:
                ...
                for fd in self.devices:
                    if fd.homeOutput.asInt > 0:
                        fd.pwr_max = fd.discharge_start + int(used * (fd.electricLevel.asInt * fd.actualKwh) / weight)
